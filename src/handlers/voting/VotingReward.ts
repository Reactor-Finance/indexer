...
VotingReward.NotifyReward.handlerWithLoader({
  ...
  handler: async ({ event, context, loaderReturn }) => {
    ...
    if (isFee) {
      ...
      const gaugeFees0CurrentEpoch = isToken0 ? pool.gaugeFees0CurrentEpoch.plus(amount) : pool.gaugeFees0CurrentEpoch;
      const gaugeFees1CurrentEpoch = !isToken0 ? pool.gaugeFees1CurrentEpoch.plus(amount) : pool.gaugeFees1CurrentEpoch;
...
VotingReward.ClaimRewards.handlerWithLoader({
  ...
  handler: async ({ event, context, loaderReturn }) => {
    ...
    if (isFee) {
      const isToken0 = token.id === pool.token0_id;
      const gaugeFees0CurrentEpoch = isToken0 ? pool.gaugeFees0CurrentEpoch.minus(amount) : pool.gaugeFees0CurrentEpoch;
